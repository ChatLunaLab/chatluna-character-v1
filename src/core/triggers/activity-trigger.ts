...
const MIN_COOLDOWN_TIME = Time.second * 6
const COOLDOWN_PENALTY = 0.8
const THRESHOLD_RESET_TIME = Time.minute * 10
...
export class ActivityTrigger extends BaseTrigger {
    readonly name = 'activity'
    readonly type = 'group' as const
    private readonly _states = new Map<string, ActivityGroupState>()
    ...
    async shouldTrigger(context: TriggerContext): Promise<TriggerResult> {
        ...
        const config = context.config.triggers.activity
        const now = context.message.timestamp ?? Date.now()
        const state = this._getState(context.guildId, config.lowerLimit)

        state.messageTimestamps.push(now)
        if (state.messageTimestamps.length > WINDOW_SIZE) {
            state.messageTimestamps.shift()
        }

        if (now - state.lastUserMessageTime >= THRESHOLD_RESET_TIME) {
            state.currentActivityThreshold = config.lowerLimit
        }
        ...
        const activity = calculateActivityScore(
            ...
            context.config.global.maxMessages,
            state.lastActivityScore,
            state.lastScoreUpdate
        )

        state.lastActivityScore = activity.score
        state.lastScoreUpdate = activity.timestamp

        const cooldownMs = Math.max(0, config.cooldownTime) * 1000
        const timeSinceResponse = state.lastResponseTime
            ? now - state.lastResponseTime
            : Infinity
        const inCooldown = timeSinceResponse < cooldownMs

        const shouldTrigger =
            activity.score >= state.currentActivityThreshold && !inCooldown

        if (shouldTrigger) {
            state.messageCount = 0
            state.lastActivityScore = Math.max(
                0,
                state.lastActivityScore - COOLDOWN_PENALTY
            )
            state.lastResponseTime = now

            const lower = config.lowerLimit
            const upper = config.upperLimit
            const step = (upper - lower) * 0.1
            state.currentActivityThreshold = clamp(
                state.currentActivityThreshold + step,
        ...
        return {
            shouldTrigger,
            priority: Math.floor(activity.score * 100),
            reason: shouldTrigger ? 'activity_threshold' : 'activity_low',
            metadata: {
                score: activity.score,
                threshold: state.currentActivityThreshold,
                cooldownRemaining: Math.max(0, cooldownMs - timeSinceResponse)
            }
        }
    }

    private _getState(guildId: string, threshold: number): ActivityGroupState {
        const existing = this._states.get(guildId)
        if (existing) {
            return existing
        }
        const now = Date.now()
        const state: ActivityGroupState = {
            ...
            lastScoreUpdate: 0,
            lastResponseTime: 0,
            currentActivityThreshold: threshold,
            lastUserMessageTime: now
        }
        this._states.set(guildId, state)
        return state
    }
}
